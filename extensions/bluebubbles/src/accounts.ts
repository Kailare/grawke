...
export function listBlueBubblesAccountIds(cfg: MoltXConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: MoltXConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: MoltXConfig): ResolvedBlueBubblesAccount[] {
