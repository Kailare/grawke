...
export function listBlueBubblesAccountIds(cfg: GrawkeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: GrawkeConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: GrawkeConfig): ResolvedBlueBubblesAccount[] {
