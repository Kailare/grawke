...
// NOTE: This extension is intended to be bundled with MoltX.
// When running from source (tests/dev), MoltX internals live under src/.
...
export function createLlmTaskTool(api: MoltXPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "moltx-llm-task-"));
