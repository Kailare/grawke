...
// NOTE: This extension is intended to be bundled with Grawke.
// When running from source (tests/dev), Grawke internals live under src/.
...
export function createLlmTaskTool(api: GrawkePluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "grawke-llm-task-"));
