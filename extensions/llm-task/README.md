...
`clawd.invoke --each`) without adding new MoltX code per workflow.
...
This extension depends on MoltX internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MoltX extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.moltx/extensions` as a standalone plugin directory.
