...
`clawd.invoke --each`) without adding new Grawke code per workflow.
...
This extension depends on Grawke internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Grawke extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.grawke/extensions` as a standalone plugin directory.
