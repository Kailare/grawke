...
metadata: {"grawke":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Grawke `sessions_spawn`
- **File I/O** == Grawke `read`/`write`
- **Remote fetch** == Grawke `web_fetch` (or `exec` with curl when POST is required)
