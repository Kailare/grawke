...
metadata: {"moltx":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == MoltX `sessions_spawn`
- **File I/O** == MoltX `read`/`write`
- **Remote fetch** == MoltX `web_fetch` (or `exec` with curl when POST is required)
