...
Grawke can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Grawke can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Grawke defaults
...
TTS config lives under `messages.tts` in `grawke.json`.
...
- If `provider` is **unset**, Grawke prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.grawke/settings/tts.json`, override with `GRAWKE_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Grawke retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Grawke registers
