...
- `/usage cost` in chats: local cost summary aggregated from MoltX session logs.
- CLI: `moltx status --usage` prints a full per-provider breakdown.
- CLI: `moltx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
