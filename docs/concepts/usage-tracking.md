...
- `/usage cost` in chats: local cost summary aggregated from Grawke session logs.
- CLI: `grawke status --usage` prints a full per-provider breakdown.
- CLI: `grawke channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
