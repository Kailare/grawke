...
Remote Gateway:
```bash
moltx tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `moltx tui --deliver`
...
- Check the Gateway logs: `moltx logs --follow`.
- Confirm the agent can run: `moltx status` and `moltx models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `moltx agents list` and your routing config.
