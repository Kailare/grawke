...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `grawke sandbox explain`.
...
Legacy `agent.*` configs are migrated by `grawke doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=grawke.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${GRAWKE_STATE_DIR:-$HOME/.grawke}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
