...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `moltx sandbox explain`.
...
Legacy `agent.*` configs are migrated by `moltx doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=moltx.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${MOLTX_STATE_DIR:-$HOME/.moltx}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
