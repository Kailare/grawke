...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Grawke would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Grawke would reply in this chat, it will run all three agents.
...
**Debug:**
```bash
tail -f ~/.grawke/logs/gateway.log | grep broadcast
```
