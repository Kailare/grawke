...
- sets a stable debug bundle identifier: `com.moltx.mac.debug`
...
- inject build metadata into Info.plist: `MoltXBuildTimestamp` (UTC) and `MoltXGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `MoltXBuildTimestamp`: ISO8601 UTC at package time
- `MoltXGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier *and* code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/MoltX.app`) preserves the grants between builds, matching the VibeTunnel approach.
