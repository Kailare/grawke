...
Tlon is a decentralized messenger built on Urbit. MoltX connects to your Urbit ship and can
...
```bash
moltx plugins install @moltx/tlon
```

Local checkout (when running from a git repo):

```bash
moltx plugins install ./extensions/tlon
```
...
Use these with `moltx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MoltX replies in-thread.
