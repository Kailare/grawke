...
Tlon is a decentralized messenger built on Urbit. Grawke connects to your Urbit ship and can
...
```bash
grawke plugins install @grawke/tlon
```

Local checkout (when running from a git repo):

```bash
grawke plugins install ./extensions/tlon
```
...
Use these with `grawke message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Grawke replies in-thread.
