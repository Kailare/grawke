...
Matrix is an open, decentralized messaging protocol. MoltX connects as a Matrix **user**
...
```bash
moltx plugins install @moltx/matrix
```

Local checkout (when running from a git repo):

```bash
moltx plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `moltx plugins install @moltx/matrix`
   - From a local checkout: `moltx plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MoltX calls the same
     login endpoint, stores the access token in `~/.moltx/credentials/matrix/credentials.json`,
...
- On first connection, MoltX requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MoltX logs a warning.
...
`~/.moltx/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `moltx pairing list matrix`
  - `moltx pairing approve matrix <CODE>`
...
- On startup, MoltX resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
