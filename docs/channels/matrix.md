...
Matrix is an open, decentralized messaging protocol. Grawke connects as a Matrix **user**
...
```bash
grawke plugins install @grawke/matrix
```

Local checkout (when running from a git repo):

```bash
grawke plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `grawke plugins install @grawke/matrix`
   - From a local checkout: `grawke plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Grawke calls the same
     login endpoint, stores the access token in `~/.grawke/credentials/matrix/credentials.json`,
...
- On first connection, Grawke requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Grawke logs a warning.
...
`~/.grawke/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `grawke pairing list matrix`
  - `grawke pairing approve matrix <CODE>`
...
- On startup, Grawke resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
