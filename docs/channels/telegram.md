...
- If Telegram rejects the HTML payload, MoltX retries the same message as plain text.

## Commands (native + custom)
MoltX registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; MoltX does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `moltx logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). MoltX can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. MoltX:
...
Private chats can include `message_thread_id` in some edge cases. MoltX keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2) Run `moltx logs --follow` and look for `from.id`.
...
MoltX defaults to audio files for backward compatibility.
...
MoltX uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, MoltX:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by MoltX)
...
- Example: `moltx message send --channel telegram --target 123456789 --message "hi"`.
...
- `moltx channels status` shows a warning when config expects unmentioned group messages.
- `moltx channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `moltx logs --follow` (look for "skipping group message")
...
- Upgrade to a MoltX build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
