...
```bash
grawke plugins install @grawke/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
grawke plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Grawke" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `grawke pairing list nextcloud-talk`
  - `grawke pairing approve nextcloud-talk <CODE>`
