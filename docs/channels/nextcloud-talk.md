...
```bash
moltx plugins install @moltx/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
moltx plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "MoltX" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `moltx pairing list nextcloud-talk`
  - `moltx pairing approve nextcloud-talk <CODE>`
