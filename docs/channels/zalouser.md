...
read_when:
  - Setting up Zalo Personal for Grawke
...
- Install via CLI: `grawke plugins install @grawke/zalouser`
- Or from a source checkout: `grawke plugins install ./extensions/zalouser`
...
Approve via:
- `grawke pairing list zalouser`
- `grawke pairing approve zalouser <code>`
...
- On startup, Grawke resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `grawke channels logout --channel zalouser && grawke channels login --channel zalouser`
