...
read_when:
  - Setting up Zalo Personal for MoltX
...
- Install via CLI: `moltx plugins install @moltx/zalouser`
- Or from a source checkout: `moltx plugins install ./extensions/zalouser`
...
Approve via:
- `moltx pairing list zalouser`
- `moltx pairing approve zalouser <code>`
...
- On startup, MoltX resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `moltx channels logout --channel zalouser && moltx channels login --channel zalouser`
