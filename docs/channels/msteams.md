...
```bash
moltx plugins install @moltx/msteams
```

Local checkout (when running from a git repo):
```bash
moltx plugins install ./extensions/msteams
```
...
3) Configure MoltX with those credentials.
...
## Goals
- Talk to MoltX via Teams DMs, group chats, or channels.
...
- On startup, MoltX resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.moltx/moltx.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring MoltX, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `moltx-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `moltx plugins install @moltx/msteams`
   - From a local checkout: `moltx plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "MoltX in Teams", "full": "MoltX in Teams" },
...
MoltX handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, MoltX only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/MoltXShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
MoltX sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `moltx message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.moltx/msteams-polls.json`.
...
**CLI:**
```bash
moltx message send --channel msteams \
...
moltx message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
moltx message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
moltx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
moltx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
