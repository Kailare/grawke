...
```bash
grawke plugins install @grawke/msteams
```

Local checkout (when running from a git repo):
```bash
grawke plugins install ./extensions/msteams
```
...
3) Configure Grawke with those credentials.
...
## Goals
- Talk to Grawke via Teams DMs, group chats, or channels.
...
- On startup, Grawke resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.grawke/grawke.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Grawke, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `grawke-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `grawke plugins install @grawke/msteams`
   - From a local checkout: `grawke plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Grawke in Teams", "full": "Grawke in Teams" },
...
Grawke handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Grawke only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/GrawkeShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Grawke sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `grawke message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.grawke/msteams-polls.json`.
...
**CLI:**
```bash
grawke message send --channel msteams \
...
grawke message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
grawke message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
grawke message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
grawke message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
