...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Grawke
---

# Node + tsx "__name is not a function" crash

## Summary
Running Grawke via Node with `tsx` fails at startup with:

```
[grawke] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `grawke status` and `gateway:watch` worked.
