...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MoltX
---

# Node + tsx "__name is not a function" crash

## Summary
Running MoltX via Node with `tsx` fails at startup with:

```
[moltx] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `moltx status` and `gateway:watch` worked.
