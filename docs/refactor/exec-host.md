...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.moltx/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.moltx/node.json` (node id + pairing token).
...
Path: `~/.moltx/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.moltx/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.moltx/exec-approvals.sock` (0600).
