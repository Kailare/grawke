...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.grawke/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.grawke/node.json` (node id + pairing token).
...
Path: `~/.grawke/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.grawke/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.grawke/exec-approvals.sock` (0600).
