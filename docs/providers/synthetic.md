---
summary: "Use Synthetic's Anthropic-compatible API in Grawke"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Grawke registers it as the
...
```bash
grawke onboard --auth-choice synthetic-api-key
```
...
Note: Grawke's Anthropic client appends `/v1` to the base URL, so use
