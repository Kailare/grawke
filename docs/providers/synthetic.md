---
summary: "Use Synthetic's Anthropic-compatible API in MoltX"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. MoltX registers it as the
...
```bash
moltx onboard --auth-choice synthetic-api-key
```
...
Note: MoltX's Anthropic client appends `/v1` to the base URL, so use
