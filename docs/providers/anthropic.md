---
summary: "Use Anthropic Claude via API keys or Claude Code CLI auth in Grawke"
read_when:
  - You want to use Anthropic models in Grawke
  - You want setup-token or Claude Code CLI auth instead of API keys
---
# Anthropic (Claude)

Anthropic builds the **Claude** model family and provides access via an API.
In Grawke you can authenticate with an API key or reuse **Claude Code CLI** credentials
...
# or non-interactive
grawke onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
Grawke does **not** override Anthropic’s default cache TTL unless you set it.
...
Grawke includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
...
Paste the token into Grawke (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
...
grawke onboard --auth-choice claude-cli
```
...
- Generate the setup-token with `claude setup-token` and paste it, or run `grawke models auth setup-token` on the gateway host.
...
- Grawke writes `auth.profiles["anthropic:claude-cli"].mode` as `"oauth"` so the profile
...
- If the Claude CLI login lives on a different machine, use
  `grawke models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**
- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `grawke models status`.

**No credentials found for profile `anthropic:default` or `anthropic:claude-cli`**
- Run `grawke models status` to see which auth profile is active.
- Re-run onboarding, or paste a setup-token / API key for that profile.

**No available auth profile (all in cooldown/unavailable)**
- Check `grawke models status --json` for `auth.unusableProfiles`.
