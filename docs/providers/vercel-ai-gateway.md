...
read_when:
  - You want to use Vercel AI Gateway with Grawke
...
```bash
grawke onboard --auth-choice ai-gateway-api-key
```
...
is available to that process (for example, in `~/.grawke/.env` or via
`env.shellEnv`).
