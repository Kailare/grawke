---
summary: "All configuration options for ~/.moltx/moltx.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

MoltX reads an optional **JSON5** config from `~/.moltx/moltx.json` (comments + trailing commas allowed).

If the file is missing, MoltX uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
MoltX only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `moltx doctor`, `moltx logs`, `moltx health`, `moltx status`, `moltx service`, `moltx help`).
- Run `moltx doctor` to see the exact issues.
- Run `moltx doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `moltx config set`. Keep a backup of `~/.moltx/moltx.json`.
...
```bash
moltx gateway call config.get --params '{}' # capture payload.hash
moltx gateway call config.apply --params '{
...
```bash
moltx gateway call config.get --params '{}' # capture payload.hash
moltx gateway call config.patch --params '{
...
{ "$include": "/etc/moltx/base.json5" } // absolute
...
```json5
// ~/.moltx/clients/mueller/agents.json5
[
...
```json5
// ~/.moltx/clients/mueller/broadcast.json5
{
...
MoltX reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, MoltX runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `MOLTX_LOAD_SHELL_ENV=1`
- `MOLTX_SHELL_ENV_TIMEOUT_MS=15000`
...
MoltX stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.moltx/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.moltx/credentials/oauth.json` (or `$MOLTX_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.moltx/agent/*` (migrated by `moltx doctor` into `~/.moltx/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `MOLTX_OAUTH_DIR`
- Agent dir (default agent root override): `MOLTX_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, MoltX imports `oauth.json` entries into `auth-profiles.json`.

MoltX also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. MoltX auto-migrates older configs that used
`mode: "token"`.
...
If set, MoltX derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/moltx/moltx-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/moltx/moltx.log`.
...
- The legacy single-account Baileys auth dir is migrated by `moltx doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "moltx", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.moltx/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `moltx.json`).
...
MoltX starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
MoltX starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `moltx plugins install @moltx/mattermost` (or `./extensions/mattermost` from a git checkout).
...
MoltX starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
MoltX spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, MoltX
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/moltx" } }
}
```
...
When a file exceeds this limit, MoltX logs a warning and injects a truncated
...
message envelopes). If unset, MoltX uses the host timezone at runtime.
...
`[moltx]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[moltx]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[moltx]"`, MoltX will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, MoltX generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.moltx/settings/tts.json",
      elevenlabs: {
...
MoltX also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, MoltX currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `moltx configure --section web`, or use `BRAVE_API_KEY` env var)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.moltx/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "moltx-sandbox-browser:bookworm-slim",
          containerPrefix: "moltx-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
MoltX uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.moltx/agents/<agentId>/agent/models.json` or by defining the same schema inside your
MoltX config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, MoltX writes/merges a `models.json` into
`~/.moltx/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. MoltX uses
...
- Shortcut: `moltx onboard --auth-choice opencode-zen`.
...
Shortcut: `moltx onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `moltx onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `moltx onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `moltx onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `moltx onboard --auth-choice minimax-api`.
...
- Override the agent config root with `MOLTX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.moltx/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.moltx/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.moltx/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, MoltX stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.moltx/skills` (workspace skills
...
from `~/.moltx/extensions`, `<workspace>/.moltx/extensions`, plus any
...
MoltX can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (MoltX.app menubar, or `moltx gateway`).
...
- When a connection comes from one of these IPs, MoltX uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `moltx gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `MOLTX_GATEWAY_PORT` > `gateway.port` > default `18789`.
- Non-loopback binds (`lan`/`tailnet`/`auto`) require auth. Use `gateway.auth.token` (or `MOLTX_GATEWAY_TOKEN`).
...
- `gateway.auth.password` can be set here, or via `MOLTX_GATEWAY_PASSWORD` (recommended).
...
macOS app behavior:
- MoltX.app watches `~/.moltx/moltx.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.moltx/moltx.json` (or `MOLTX_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.moltx/moltx.json` (or `MOLTX_CONFIG_PATH`)
...
- `MOLTX_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `moltx --dev …` → uses `~/.moltx-dev` + shifts ports from base `19001`
- `moltx --profile <name> …` → uses `~/.moltx-<name>` (port via config/env/flags)
...
Example:
```bash
MOLTX_CONFIG_PATH=~/.moltx/a.json \
MOLTX_STATE_DIR=~/.moltx-a \
...
Gmail helper config (used by `moltx webhooks gmail setup` / `run`):
...
- Set `MOLTX_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, MoltX defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__moltx/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__moltx__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.moltx/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.moltx/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.moltx/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_moltx-bridge._tcp` under `~/.moltx/dns/` using the standard discovery domain `moltx.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `moltx.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `moltx.internal` via that server
