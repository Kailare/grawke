---
summary: "All configuration options for ~/.grawke/grawke.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Grawke reads an optional **JSON5** config from `~/.grawke/grawke.json` (comments + trailing commas allowed).

If the file is missing, Grawke uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Grawke only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `grawke doctor`, `grawke logs`, `grawke health`, `grawke status`, `grawke service`, `grawke help`).
- Run `grawke doctor` to see the exact issues.
- Run `grawke doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `grawke config set`. Keep a backup of `~/.grawke/grawke.json`.
...
```bash
grawke gateway call config.get --params '{}' # capture payload.hash
grawke gateway call config.apply --params '{
...
```bash
grawke gateway call config.get --params '{}' # capture payload.hash
grawke gateway call config.patch --params '{
...
{ "$include": "/etc/grawke/base.json5" } // absolute
...
```json5
// ~/.grawke/clients/mueller/agents.json5
[
...
```json5
// ~/.grawke/clients/mueller/broadcast.json5
{
...
Grawke reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Grawke runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `GRAWKE_LOAD_SHELL_ENV=1`
- `GRAWKE_SHELL_ENV_TIMEOUT_MS=15000`
...
Grawke stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.grawke/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.grawke/credentials/oauth.json` (or `$GRAWKE_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.grawke/agent/*` (migrated by `grawke doctor` into `~/.grawke/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `GRAWKE_OAUTH_DIR`
- Agent dir (default agent root override): `GRAWKE_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Grawke imports `oauth.json` entries into `auth-profiles.json`.

Grawke also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. Grawke auto-migrates older configs that used
`mode: "token"`.
...
If set, Grawke derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/grawke/grawke-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/grawke/grawke.log`.
...
- The legacy single-account Baileys auth dir is migrated by `grawke doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "grawke", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.grawke/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `grawke.json`).
...
Grawke starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Grawke starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `grawke plugins install @grawke/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Grawke starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Grawke spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Grawke
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/grawke" } }
}
```
...
When a file exceeds this limit, Grawke logs a warning and injects a truncated
...
message envelopes). If unset, Grawke uses the host timezone at runtime.
...
`[grawke]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[grawke]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[grawke]"`, Grawke will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Grawke generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.grawke/settings/tts.json",
      elevenlabs: {
...
Grawke also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Grawke currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `grawke configure --section web`, or use `BRAVE_API_KEY` env var)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.grawke/sandboxes`
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "grawke-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "grawke-sandbox-browser:bookworm-slim",
          containerPrefix: "grawke-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Grawke uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.grawke/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Grawke config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Grawke writes/merges a `models.json` into
`~/.grawke/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Grawke uses
...
- Shortcut: `grawke onboard --auth-choice opencode-zen`.
...
Shortcut: `grawke onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `grawke onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `grawke onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `grawke onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `grawke onboard --auth-choice minimax-api`.
...
- Override the agent config root with `GRAWKE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.grawke/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.grawke/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.grawke/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Grawke stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.grawke/skills` (workspace skills
...
from `~/.grawke/extensions`, `<workspace>/.grawke/extensions`, plus any
...
Grawke can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (Grawke.app menubar, or `grawke gateway`).
...
- Examples: `"/ui"`, `"/grawke"`, `"/apps/grawke"`.
...
- When a connection comes from one of these IPs, Grawke uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `grawke gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `GRAWKE_GATEWAY_PORT` > `gateway.port` > default `18789`.
- Non-loopback binds (`lan`/`tailnet`/`auto`) require auth. Use `gateway.auth.token` (or `GRAWKE_GATEWAY_TOKEN`).
...
- `gateway.auth.password` can be set here, or via `GRAWKE_GATEWAY_PASSWORD` (recommended).
...
macOS app behavior:
- Grawke.app watches `~/.grawke/grawke.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.grawke/grawke.json` (or `GRAWKE_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.grawke/grawke.json` (or `GRAWKE_CONFIG_PATH`)
...
- `GRAWKE_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `grawke --dev …` → uses `~/.grawke-dev` + shifts ports from base `19001`
- `grawke --profile <name> …` → uses `~/.grawke-<name>` (port via config/env/flags)
...
Example:
```bash
GRAWKE_CONFIG_PATH=~/.grawke/a.json \
GRAWKE_STATE_DIR=~/.grawke-a \
...
Gmail helper config (used by `grawke webhooks gmail setup` / `run`):
...
- Set `GRAWKE_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Grawke defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__grawke/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__grawke__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.grawke/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.grawke/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.grawke/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_grawke-bridge._tcp` under `~/.grawke/dns/` using the standard discovery domain `grawke.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `grawke.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `grawke.internal` via that server
