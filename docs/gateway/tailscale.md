...
Grawke can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Grawke requires a shared password.
...
- `token` (default when `GRAWKE_GATEWAY_TOKEN` is set)
- `password` (shared secret via `GRAWKE_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Grawke only
...
Prefer `GRAWKE_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
grawke gateway --tailscale serve
grawke gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Grawke to undo `tailscale serve`
