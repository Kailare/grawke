...
MoltX can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. MoltX requires a shared password.
...
- `token` (default when `MOLTX_GATEWAY_TOKEN` is set)
- `password` (shared secret via `MOLTX_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. MoltX only
...
Prefer `MOLTX_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
moltx gateway --tailscale serve
moltx gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want MoltX to undo `tailscale serve`
