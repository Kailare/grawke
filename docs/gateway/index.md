...
- Replaces the legacy `gateway` command. CLI entry point: `grawke gateway`.
...
grawke gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.grawke/grawke.json` (or `GRAWKE_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__grawke__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `GRAWKE_SKIP_CANVAS_HOST=1`.
...
- Gateway auth: set `gateway.auth.mode=token` + `gateway.auth.token` (or pass `--token <value>` / `GRAWKE_GATEWAY_TOKEN`) to require clients to send `connect.params.auth.token`.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `GRAWKE_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.grawke.<profile>`
- Linux: `grawke-gateway-<profile>.service`
- Windows: `Grawke Gateway (<profile>)`

Install metadata is embedded in the service config:
- `GRAWKE_SERVICE_MARKER=grawke`
- `GRAWKE_SERVICE_KIND=gateway`
...
```bash
grawke --dev setup
grawke --dev gateway --allow-unconfigured
...
- `GRAWKE_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- `browser.controlUrl port = base + 2` (or `GRAWKE_BROWSER_CONTROL_URL` / config override)
- `canvasHost.port = base + 4` (or `GRAWKE_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `GRAWKE_CONFIG_PATH`
- unique `GRAWKE_STATE_DIR`
...
Example:
```bash
GRAWKE_CONFIG_PATH=~/.grawke/a.json GRAWKE_STATE_DIR=~/.grawke-a grawke gateway --port 19001
GRAWKE_CONFIG_PATH=~/.grawke/b.json GRAWKE_STATE_DIR=~/.grawke-b grawke gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `grawke health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `grawke`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `grawke gateway install` writes `~/Library/LaunchAgents/com.grawke.gateway.plist`
    (or `com.grawke.<profile>.plist`).
  - `grawke doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Grawke profile services.
  ...
  - Cleanup: `grawke gateway uninstall` (current service) and `grawke doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `grawke gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Grawke.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.grawke.gateway` (or `com.grawke.<profile>`).
- To stop it cleanly, use `grawke gateway stop` (or `launchctl bootout gui/$UID/com.grawke.gateway`).
- To restart, use `grawke gateway restart` (or `launchctl kickstart -k gui/$UID/com.grawke.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `grawke gateway install` first.
  - Replace the label with `com.grawke.<profile>` when running a named profile.

## Supervision (systemd user unit)
Grawke installs a **systemd user service** by default on Linux/WSL2. We
...
`grawke gateway install` writes the user unit. `grawke doctor` audits the
...
Restart=always
RestartSec=5
Environment=GRAWKE_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/grawke-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now grawke-gateway[-<profile>].service
```
...
## CLI helpers
- `grawke gateway health|status` — request health/status over the Gateway WS.
- `grawke message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `grawke agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `grawke gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `grawke gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `grawke gateway` and the legacy TCP control port.
