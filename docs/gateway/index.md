...
- Replaces the legacy `gateway` command. CLI entry point: `moltx gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.moltx/moltx.json` (or `MOLTX_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__moltx__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `MOLTX_SKIP_CANVAS_HOST=1`.
...
- Gateway auth: set `gateway.auth.mode=token` + `gateway.auth.token` (or pass `--token <value>` / `MOLTX_GATEWAY_TOKEN`) to require clients to send `connect.params.auth.token`.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `MOLTX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.moltx.<profile>`
- Linux: `moltx-gateway-<profile>.service`
- Windows: `MoltX Gateway (<profile>)`

Install metadata is embedded in the service config:
- `MOLTX_SERVICE_MARKER=moltx`
- `MOLTX_SERVICE_KIND=gateway`
- `MOLTX_SERVICE_VERSION=<version>`
...
```bash
moltx --dev setup
moltx --dev gateway --allow-unconfigured
...
- `MOLTX_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- `browser.controlUrl port = base + 2` (or `MOLTX_BROWSER_CONTROL_URL` / config override)
- `canvasHost.port = base + 4` (or `MOLTX_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `MOLTX_CONFIG_PATH`
- unique `MOLTX_STATE_DIR`
...
Example:
```bash
MOLTX_CONFIG_PATH=~/.moltx/a.json MOLTX_STATE_DIR=~/.moltx-a moltx gateway --port 19001
MOLTX_CONFIG_PATH=~/.moltx/b.json MOLTX_STATE_DIR=~/.moltx-b moltx gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `moltx health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `moltx`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `moltx gateway install` writes `~/Library/LaunchAgents/com.moltx.gateway.plist`
    (or `com.moltx.<profile>.plist`).
  - `moltx doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are MoltX profile services.
  ...
  - Cleanup: `moltx gateway uninstall` (current service) and `moltx doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `moltx gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- MoltX.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.moltx.gateway` (or `com.moltx.<profile>`).
- To stop it cleanly, use `moltx gateway stop` (or `launchctl bootout gui/$UID/com.moltx.gateway`).
- To restart, use `moltx gateway restart` (or `launchctl kickstart -k gui/$UID/com.moltx.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `moltx gateway install` first.
  - Replace the label with `com.moltx.<profile>` when running a named profile.

## Supervision (systemd user unit)
MoltX installs a **systemd user service** by default on Linux/WSL2. We
...
`moltx gateway install` writes the user unit. `moltx doctor` audits the
...
Restart=always
RestartSec=5
Environment=MOLTX_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/moltx-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now moltx-gateway[-<profile>].service
```
...
## CLI helpers
- `moltx gateway health|status` — request health/status over the Gateway WS.
- `moltx message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `moltx agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `moltx gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `moltx gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `moltx gateway` and the legacy TCP control port.
