...
- Jobs persist under `~/.moltx/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.moltx/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.moltx/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
moltx cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
moltx cron edit <jobId> --agent ops
moltx cron edit <jobId> --clear-agent
```
```
...
```bash
moltx system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`moltx system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `MOLTX_SKIP_CRON`.
