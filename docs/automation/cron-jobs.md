...
- Jobs persist under `~/.grawke/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.grawke/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.grawke/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
grawke cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
grawke cron edit <jobId> --agent ops
grawke cron edit <jobId> --clear-agent
```
```
...
```bash
grawke system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`grawke system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `GRAWKE_SKIP_CRON`.
