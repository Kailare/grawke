---
summary: "Agent tool surface for MoltX (browser, canvas, nodes, message, cron) replacing legacy `moltx-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `moltx-*` skills
---

# Tools (MoltX)

MoltX exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `moltx-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `moltx.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, MoltX logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`moltx node run`).
...
Notes:
- Requires a Brave API key (recommended: `moltx configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `moltx nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `moltx gateway` restart in-place)
