...
- `gateway`/`node` approvals are controlled by `~/.grawke/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Grawke prepends `env.PATH` after profile sourcing; `tools.exec.pathPrepend` applies here too.
