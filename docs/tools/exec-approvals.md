...
- **gateway host** → `moltx` process on the gateway machine
...
`~/.moltx/exec-approvals.json` directly.

CLI: `moltx approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
