---
summary: "Frequently asked questions about MoltX setup, configuration, and usage"
---
# FAQ
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What’s the recommended way to install and set up MoltX?](#whats-the-recommended-way-to-install-and-set-up-moltx)
  ...
  - [I can't access docs.clawd.bot (SSL error). What now?](#i-cant-access-docsmoltx-ssl-error-what-now)
  - [What’s the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or moltx not recognized](#windows-install-says-git-not-found-or-moltx-not-recognized)
  - [The docs didn’t answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install MoltX on Linux?](#how-do-i-install-moltx-on-linux)
  - [How do I install MoltX on a VPS?](#how-do-i-install-moltx-on-a-vps)
  ...
  - [If I buy a Mac mini to run MoltX, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-moltx-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different MoltXs?](#can-multiple-people-use-one-whatsapp-number-with-different-moltxs)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What’s the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run MoltX on a dedicated machine?](#how-important-is-it-to-run-moltx-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run MoltX in a VM and what are the requirements](#can-i-run-moltx-in-a-vm-and-what-are-the-requirements)
- [What is MoltX?](#what-is-moltx)
  - [What is MoltX, in one paragraph?](#what-is-moltx-in-one-paragraph)
  - [What’s the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for MoltX](#what-are-the-top-five-everyday-use-cases-for-moltx)
  - [Can MoltX help with lead gen outreach ads and blogs for a SaaS](#can-moltx-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can MoltX run tasks on a schedule or continuously in the background?](#can-moltx-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with MoltX saved locally?](#is-all-data-used-with-moltx-saved-locally)
  - [Where does MoltX store its data?](#where-does-moltx-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What’s the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall MoltX?](#how-do-i-completely-uninstall-moltx)
...
- [Config basics](#config-basics)
  ...
  - [Can the MoltX browser run headless?](#can-the-moltx-browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two MoltXs talk to each other (local + VPS)?](#can-two-moltxs-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does MoltX load environment variables?](#how-does-moltx-load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of MoltXs one CEO and many agents](#is-there-a-way-to-make-a-team-of-moltxs-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset MoltX but keep it installed?](#how-do-i-completely-reset-moltx-but-keep-it-installed)
  ...
  - [Why doesn’t MoltX reply in a group?](#why-doesnt-moltx-reply-in-a-group)
...
- [Gateway: ports, “already running”, and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `moltx gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-moltx-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `moltx gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-moltx-gateway-status-show-config-cli-and-config-service-different)
  - [What does “another gateway instance is already listening” mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run MoltX in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-moltx-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart MoltX?](#i-closed-my-terminal-on-windows-how-do-i-restart-moltx)
  ...
  - [ELI5: `moltx gateway restart` vs `moltx gateway`](#eli5-moltx-gateway-restart-vs-moltx-gateway)
  - [What’s the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose MoltX to inbound DMs?](#is-it-safe-to-expose-moltx-to-inbound-dms)
...
5) **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/moltx/moltx-*.log | head -1)"
   ```
...
7) **Gateway snapshot**
   ```bash
   moltx health --json
   moltx health --verbose   # shows the target URL + config path on errors
   ```
...
This installs MoltX **from a git checkout**, so the agent can read the code + docs and
...
What they do:
- `moltx status`: quick snapshot of gateway/agent health + basic config.
- `moltx models status`: checks provider auth + model availability.
- `moltx doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `moltx status --all`, `moltx logs --follow`,
`moltx gateway status`, `moltx health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### Whats the recommended way to install and set up MoltX
...
If you don’t have a global install yet, run it via `pnpm moltx onboard`.
...
- If it asks for auth, run `moltx dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `MOLTX_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**
- **Tailscale Serve** (recommended): keep bind loopback, run `moltx gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `moltx gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from `moltx dashboard`.
...
1) Install MoltX on the new machine.
2) Copy `$MOLTX_STATE_DIR` (default: `~/.moltx`) from the old machine.
3) Copy your workspace (default: `~/clawd`).
4) Run `moltx doctor` and restart the Gateway service.
...
under `~/.moltx/` (for example `~/.moltx/agents/<agentId>/sessions/`).

Related: [Where things live on disk](/help/faq#where-does-moltx-store-its-data),
...
### I cant access docsmoltx SSL error What now
...
detail: [Troubleshooting](/help/troubleshooting#docsmoltx-shows-an-ssl-error-comcastxfinity).
...
### Windows install says git not found or moltx not recognized
...
**2) moltx is not recognized after install**
...
### How do I install MoltX on Linux
...
### How do I install MoltX on a VPS
...
moltx update
moltx update status
moltx update --channel stable|beta|dev
moltx update --tag <dist-tag|version>
...
`moltx onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run MoltX with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. If Claude Code CLI credentials are present on the gateway host, MoltX can reuse them; otherwise choose **Anthropic token (paste setup-token)** and paste the string. The token is stored as an auth profile for the **anthropic** provider and used like an API key or OAuth profile. More detail: [OAuth](/concepts/oauth).

MoltX keeps `auth.profiles["anthropic:claude-cli"].mode` set to `"oauth"` so
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `moltx models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `moltx models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth Claude Code OAuth

Yes. MoltX can **reuse Claude Code CLI credentials** (OAuth) and also supports **setup-token**. If you have a Claude subscription, we recommend **setup-token** for long‑running setups (requires Claude Pro/Max + the `claude` CLI). You can generate it anywhere and paste it on the gateway host. OAuth reuse is supported, but avoid logging in separately via MoltX and Claude Code to prevent token conflicts. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so MoltX can keep replying while a provider is rate‑limited.
...
MoltX supports **OpenAI Code (Codex)** via OAuth or by reusing your Codex CLI login (`~/.codex/auth.json`). The wizard can import the CLI login or run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. MoltX fully supports **OpenAI Code (Codex) subscription OAuth** and can also reuse an
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `moltx.json`.

Steps:
1) Enable the plugin: `moltx plugins enable google-gemini-cli-auth`
2) Login: `moltx models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. MoltX needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. MoltX runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
any Mac works. MoltX’s iMessage integrations run on macOS (BlueBubbles or `imsg`), while
...
### If I buy a Mac mini to run MoltX can I connect it to my MacBook Pro
...
- Use `moltx nodes status` / `moltx nodes list` to see it.
...
- DM your bot, then run `moltx logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different MoltXs
...
If you run MoltX via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non‑login shells.
...
This **does not delete your data** - it only changes the MoltX code install. Your state
(`~/.moltx`) and workspace (`~/clawd`) stay untouched.
...
**MoltX-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run MoltX on a dedicated machine
...
MoltX is lightweight. For a basic Gateway + one chat channel:
...
### Can I run MoltX in a VM and what are the requirements
...
### What is MoltX in one paragraph

MoltX is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### Whats the value proposition

MoltX is not “just a Claude wrapper.” It’s a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for MoltX
...
### Can MoltX help with lead gen outreach ads and blogs for a SaaS
...
MoltX draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

MoltX is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use MoltX when you
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.moltx/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.moltx/moltx.json`). Precedence is `<workspace>/skills` > `~/.moltx/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.moltx/moltx.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.moltx/skills` → bundled → `skills.load.extraDirs`. `clawdhub` installs into `./skills` by default, which MoltX treats as `<workspace>/skills`.
...
Checklist:
- Confirm cron is enabled (`cron.enabled`) and `MOLTX_SKIP_CRON` is not set.
- Check the Gateway is running 24/7 (no sleep/restarts).
- Verify timezone settings for the job (`--tz` vs host timezone).

Debug:
```bash
moltx cron run <jobId> --force
moltx cron runs --id <jobId> --limit 50
```
...
### Can MoltX run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.moltx.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**  
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-moltx-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**  
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. MoltX can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3) Override the skill metadata (workspace or `~/.moltx/skills`) to allow Linux:
   ```markdown
   ---
   name: imsg
   description: iMessage/SMS CLI for listing chats, history, watch, and sending.
   metadata: {"moltx":{"os":["darwin","linux"],"requires":{"bins":["imsg"]}}}
   ---
   ```
4) Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (MoltX only needs stdio). See [iMessage](/channels/imessage).
...
ClawdHub installs into `./skills` under your current directory (or falls back to your configured MoltX workspace); MoltX treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.moltx/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawdHub](/tools/clawdhub).
...
MoltX memory is just Markdown files in the agent workspace:
- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

MoltX also runs a **silent pre-compaction memory flush** to remind the model
...
If you don’t set a provider explicitly, MoltX auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, MoltX
prefers `local`.
...
### Is all data used with MoltX saved locally

No - **MoltX’s state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.moltx` + your workspace directory).
...
### Where does MoltX store its data

Everything lives under `$MOLTX_STATE_DIR` (default: `~/.moltx`):

| Path | Purpose |
|------|---------|
| `$MOLTX_STATE_DIR/moltx.json` | Main config (JSON5) |
| `$MOLTX_STATE_DIR/credentials/oauth.json` | Legacy OAuth import (copied into auth profiles on first use) |
| `$MOLTX_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys) |
| `$MOLTX_STATE_DIR/agents/<agentId>/agent/auth.json` | Runtime auth cache (managed automatically) |
| `$MOLTX_STATE_DIR/credentials/` | Provider state (e.g. `whatsapp/<accountId>/creds.json`) |
| `$MOLTX_STATE_DIR/agents/` | Per‑agent state (agentDir + sessions) |
| `$MOLTX_STATE_DIR/agents/<agentId>/sessions/` | Conversation history & state (per agent) |
| `$MOLTX_STATE_DIR/agents/<agentId>/sessions/sessions.json` | Session metadata (per agent) |

Legacy single‑agent path: `~/.moltx/agent/*` (migrated by `moltx doctor`).
...
These files live in the **agent workspace**, not `~/.moltx`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.moltx`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.moltx/skills`).
...
Do **not** commit anything under `~/.moltx` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall MoltX
...
`workspace` to the repo root. The MoltX repo is just source code; keep the
...
MoltX reads an optional **JSON5** config from `$MOLTX_CONFIG_PATH` (default: `~/.moltx/moltx.json`):
...
Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `MOLTX_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `moltx doctor --generate-gateway-token`.
...
key. **Recommended:** run `moltx configure --section web` to store it in
...
- Daemons read env vars from `~/.moltx/.env` (or the service environment).
...
### Can the MoltX browser run headless
...
Check the basics:
- Gateway is running: `moltx gateway status`
- Gateway health: `moltx status`
- Channel health: `moltx channels status`
...
### Can two MoltXs talk to each other local VPS
...
`moltx agent --message ... --deliver`, targeting a chat where the other bot
listens. If one bot is on Railway/VPS, point your CLI at that remote Gateway
via SSH/Tailscale (see [Remote access](/gateway/remote)).

Example pattern (run from a machine that can reach the target Gateway):
```bash
moltx agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- **Local browser automation.** Keep the Gateway on a VPS, but run Chrome locally and relay control
  with the Chrome extension + `moltx browser serve`.
...
Recover:
- Restore from backup (git or a copied `~/.moltx/moltx.json`).
- If you have no backup, re-run `moltx doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:
- Use `moltx config set` for small changes.
- Use `moltx configure` for interactive edits.
...
### How does MoltX load environment variables

MoltX reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.moltx/.env` (aka `$MOLTX_STATE_DIR/.env`)
...
1) Put the missing keys in `~/.moltx/.env` so they’re picked up even when the service doesn’t inherit your shell env.
...
`MOLTX_LOAD_SHELL_ENV=1`, `MOLTX_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`moltx models status` reports whether **shell env import** is enabled. “Shell env: off”
does **not** mean your env vars are missing - it just means MoltX won’t load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won’t inherit your shell
environment. Fix by doing one of these:

1) Put the token in `~/.moltx/.env`:
   ```
...
### Is there a way to make a team of MoltXs one CEO and many agents
...
### How do I completely reset MoltX but keep it installed
...
```bash
moltx reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `MOLTX_PROFILE`), reset each state dir (defaults are `~/.moltx-<profile>`).
- Dev reset: `moltx gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), MoltX skips the heartbeat run to save API calls.
...
No. MoltX runs on **your own account**, so if you’re in the group, MoltX can see it.
...
```bash
moltx directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt MoltX reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.moltx/agents/<agentId>/sessions/`.
...
- Use `moltx doctor` to spot stray workspaces and profile mismatches.
...
MoltX’s default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, MoltX currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `moltx models set ...` (updates just model config)
- `moltx configure --section models` (interactive)
- edit `agents.defaults.model` in `~/.moltx/moltx.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `moltx doctor` to repair.
...
Yes. MoltX ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Fix options:
- Run `moltx agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so MoltX can keep responding even when a provider is rate‑limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.moltx/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.moltx/agent/*` (migrated by `moltx doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.moltx/.env` or enable `env.shellEnv`.
- **Make sure you’re editing the correct agent**
  - Multi‑agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity‑check model/auth status**
  - Use `moltx models status` to see configured models and whether providers are authenticated.
...
- **Sync the Claude Code CLI token on the gateway host**
  - Run `moltx models status` (it loads + syncs Claude Code CLI credentials).
  - If it still says missing: run `claude setup-token` (or `moltx models auth setup-token --provider anthropic`) and retry.
- **If the token was created on another machine**
  - Paste it into the gateway host with `moltx models auth paste-token --provider anthropic`.
- **Check the profile mode**
  - `auth.profiles["anthropic:claude-cli"].mode` must be `"oauth"` (token mode rejects OAuth credentials).
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.moltx/.env` on the **gateway host**.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), MoltX will try it during model fallback. If you haven’t configured Google credentials, you’ll see `No API key found for provider "google"`.
...
Fix: MoltX now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
MoltX uses provider‑prefixed IDs like:
...
MoltX may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `moltx models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
moltx models auth order set --provider anthropic anthropic:claude-cli anthropic:default
...
```bash
moltx models auth order set --provider anthropic --agent main anthropic:claude-cli
```
...
```
--port > MOLTX_GATEWAY_PORT > gateway.port > default 18789
```

### Why does moltx gateway status say Runtime running but RPC probe failed

Because “running” is the **supervisor’s** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `moltx gateway status` and trust these lines:
- `Probe target:` (the URL the probe actually used)
- `Listening:` (what’s actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn’t listening)

### Why does moltx gateway status show Config cli and Config service different

You’re editing one config file while the service is running another (often a `--profile` / `MOLTX_STATE_DIR` mismatch).

Fix:
```bash
moltx gateway install --force
```
Run that from the same `--profile` / environment you want the service to use.

### What does another gateway instance is already listening mean

MoltX enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `moltx gateway --port <port>`.

### How do I run MoltX in remote mode client connects to a Gateway elsewhere
...
Notes:
- `moltx gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
Facts (from code):
- The Control UI stores the token in browser localStorage key `moltx.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:
- Fastest: `moltx dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `moltx doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`.
- Set `gateway.auth.token` (or `MOLTX_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token (or refresh with a one-time `?token=...` link).
- Still stuck? Run `moltx status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `MOLTX_CONFIG_PATH` (per‑instance config)
- `MOLTX_STATE_DIR` (per‑instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):
- Use `moltx --profile <name> …` per instance (auto-creates `~/.moltx-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `moltx --profile <name> gateway install`.

Profiles also suffix service names (`com.moltx.<profile>`, `moltx-gateway-<profile>.service`, `MoltX Gateway (<profile>)`).
...
```
moltx tui --url ws://<host>:18789 --token <token>
```
...
- macOS: `$MOLTX_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.moltx/logs/...`; profiles use `~/.moltx-<profile>/logs/...`)
- Linux: `journalctl --user -u moltx-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "MoltX Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `moltx gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart MoltX
...
1) Is the Gateway running? `moltx gateway status`
2) Is the Gateway healthy? `moltx status`
3) Does the UI have the right token? `moltx dashboard`
...
```bash
moltx channels status
moltx channels logs --channel telegram
```
...
### ELI5 moltx gateway restart vs moltx gateway

- `moltx gateway restart`: restarts the **background service** (launchd/systemd).
- `moltx gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `moltx gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [MoltX assistant setup](/start/clawd) and [Agent send](/tools/agent-send).

CLI sending:

```bash
moltx message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose MoltX to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM‑capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `moltx pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `moltx pairing list <channel>` if a code didn’t arrive.
- Opening DMs publicly requires explicit opt‑in (`dmPolicy: "open"` and allowlist `"*"`).

Run `moltx doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. MoltX only replies to chats it receives or to explicit sends you trigger.
...
MoltX blocks **cross‑provider** messaging by default. If a tool call is bound
