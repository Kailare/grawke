...
Deploy MoltX on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.app/new/template?template=https://github.com/vignesh07/moltx-railway-template" target="_blank" rel="noreferrer">Deploy on Railway</a>
...
- `https://<your-railway-domain>/moltx` — Control UI

## What you get

- Hosted MoltX Gateway + Control UI
...
- `MOLTX_STATE_DIR=/data/.moltx` (recommended)
- `MOLTX_WORKSPACE_DIR=/data/workspace` (recommended)
- `MOLTX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your MoltX state + workspace so you can migrate to another host without losing config or memory.
