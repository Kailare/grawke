...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/grawke`)
...
If the page fails to load, start the Gateway first: `grawke gateway`.
...
- Config: view/edit `~/.grawke/grawke.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Grawke
...
```bash
grawke gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Grawke **blocks** Control UI connections without device identity.
...
```bash
GRAWKE_CONTROL_UI_BASE_PATH=/grawke/ pnpm ui:build
```
