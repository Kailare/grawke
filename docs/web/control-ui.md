...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/moltx`)
...
If the page fails to load, start the Gateway first: `moltx gateway`.
...
- Config: view/edit `~/.moltx/moltx.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. MoltX
...
```bash
moltx gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
MoltX **blocks** Control UI connections without device identity.
...
```bash
MOLTX_CONTROL_UI_BASE_PATH=/moltx/ pnpm ui:build
```
