...
read_when:
  - You installed MoltX but `moltx` is “command not found”
...
MoltX’s runtime baseline is **Node 22+**.

If you can run `npm install -g moltx@latest` but later see `moltx: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
