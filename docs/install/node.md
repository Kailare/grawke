...
read_when:
  - You installed Grawke but `grawke` is “command not found”
...
Grawke’s runtime baseline is **Node 22+**.

If you can run `npm install -g grawke@latest` but later see `grawke: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
