...
read_when:
  ...
  - Deciding: standalone CLI vs deep MoltX integration
...
Recommendation: **deep integration in MoltX**, but keep a separable core library.

### Why integrate into MoltX?
- MoltX already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `moltx memory recall "…" --k 25 --since 30d`
  - `moltx memory reflect --since 7d`
