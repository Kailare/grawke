...
read_when:
  ...
  - Deciding: standalone CLI vs deep Grawke integration
...
Recommendation: **deep integration in Grawke**, but keep a separable core library.

### Why integrate into Grawke?
- Grawke already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `grawke memory recall "…" --k 25 --since 30d`
  - `grawke memory reflect --since 7d`
