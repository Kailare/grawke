...
import { CONFIG_PATH_MOLTX, loadConfig, readConfigFileSnapshot, resolveGatewayPort, } from "../../config/config.js";
...
async function runGatewayCommand(opts) {
    const isDevProfile = process.env.MOLTX_PROFILE?.trim().toLowerCase() === "dev";
    ...
    if (opts.claudeCliLogs) {
        ...
        process.env.MOLTX_CLAUDE_CLI_LOG_OUTPUT = "1";
    }
    ...
    if (opts.rawStream) {
        process.env.MOLTX_RAW_STREAM = "1";
    }
    const rawStreamPath = toOptionString(opts.rawStreamPath);
    if (rawStreamPath) {
        process.env.MOLTX_RAW_STREAM_PATH = rawStreamPath;
    }
    if (devMode) {
    ...
    const configExists = fs.existsSync(CONFIG_PATH_MOLTX);
    const mode = cfg.gateway?.mode;
    if (!opts.allowUnconfigured && mode !== "local") {
        if (!configExists) {
            defaultRuntime.error(`Missing config. Run \`${formatCliCommand("moltx setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`);
        }
        else {
    ...
    if (resolvedAuthMode === "token" && !tokenValue) {
        defaultRuntime.error([
            ...
            "Set gateway.auth.token (or MOLTX_GATEWAY_TOKEN), or pass --token.",
            ...authHints,
        ]
            .filter(Boolean)
            .join("\n"));
    ...
    if (resolvedAuthMode === "password" && !passwordValue) {
        defaultRuntime.error([
            ...
            "Set gateway.auth.password (or MOLTX_GATEWAY_PASSWORD), or pass --password.",
            ...authHints,
        ]
            .filter(Boolean)
            .join("\n"));
    ...
    try {
        ...
        if (err instanceof GatewayLockError ||
            ...
            defaultRuntime.error(`Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("moltx gateway stop")}`);
            try {
...
export function addGatewayRunCommand(cmd) {
    return cmd
        ...
        .option("--token <token>", "Shared token required in connect.params.auth.token (default: MOLTX_GATEWAY_TOKEN env if set)")
