...
import { CONFIG_PATH_GRAWKE, writeConfigFile } from "../../config/config.js";
...
const resolveDevWorkspaceDir = (env = process.env) => {
    ...
    const profile = env.GRAWKE_PROFILE?.trim().toLowerCase();
    if (profile === "dev")
        return baseDir;
...
async function ensureDevWorkspace(dir) {
    ...
    const [agents, soul, tools, identity, user] = await Promise.all([
        loadDevTemplate("AGENTS.dev.md", `# AGENTS.md - Grawke Dev Workspace\n\nDefault dev workspace for grawke gateway --dev.\n`),
...
export async function ensureDevGatewayConfig(opts) {
    ...
    defaultRuntime.log(`Dev config ready: ${shortenHomePath(CONFIG_PATH_GRAWKE)}`);
