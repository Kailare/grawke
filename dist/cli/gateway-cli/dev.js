...
import { CONFIG_PATH_MOLTX, writeConfigFile } from "../../config/config.js";
...
const resolveDevWorkspaceDir = (env = process.env) => {
    ...
    const profile = env.MOLTX_PROFILE?.trim().toLowerCase();
    if (profile === "dev")
        return baseDir;
...
async function ensureDevWorkspace(dir) {
    ...
    const [agents, soul, tools, identity, user] = await Promise.all([
        loadDevTemplate("AGENTS.dev.md", `# AGENTS.md - MoltX Dev Workspace\n\nDefault dev workspace for moltx gateway --dev.\n`),
...
export async function ensureDevGatewayConfig(opts) {
    ...
    defaultRuntime.log(`Dev config ready: ${shortenHomePath(CONFIG_PATH_MOLTX)}`);
