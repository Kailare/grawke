...
export function registerSecurityCli(program) {
    ...
    security
        ...
        const lines = [];
        lines.push(heading("MoltX security audit"));
        lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
        lines.push(muted(`Run deeper: ${formatCliCommand("moltx security audit --deep")}`));
        if (opts.fix) {
            lines.push(muted(`Fix: ${formatCliCommand("moltx security audit --fix")}`));
            if (!fixResult) {
