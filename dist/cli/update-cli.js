...
const STEP_LABELS = {
    ...
    "moltx doctor": "Running doctor checks",
...
const MOLTX_REPO_URL = "https://github.com/Kailare/moltx.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "moltx");
...
async function isMoltXPackage(root) {
    try {
        const raw = await fs.readFile(path.join(root, "package.json"), "utf-8");
        const parsed = JSON.parse(raw);
        return parsed?.name === "moltx";
    }
    catch {
        return false;
    }
...
function resolveGitInstallDir() {
    const override = process.env.MOLTX_GIT_DIR?.trim();
    if (override)
...
async function ensureGitCheckout(params) {
    ...
    if (!dirExists) {
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", MOLTX_REPO_URL, params.dir],
            timeoutMs: params.timeoutMs,
    ...
    if (!(await isGitCheckout(params.dir))) {
        ...
        if (!empty) {
            throw new Error(`MOLTX_GIT_DIR points at a non-git directory: ${params.dir}. Set MOLTX_GIT_DIR to an empty folder or a moltx checkout.`);
        }
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", MOLTX_REPO_URL, params.dir],
    ...
    if (!(await isMoltXPackage(params.dir))) {
        throw new Error(`MOLTX_GIT_DIR does not look like a moltx checkout: ${params.dir}.`);
    }
    return null;
}
...
export async function updateStatusCommand(opts) {
    ...
    defaultRuntime.log(theme.heading("MoltX update status"));
    defaultRuntime.log("");
...
export async function updateCommand(opts) {
    ...
    if (!opts.json) {
        defaultRuntime.log(theme.heading("Updating MoltX..."));
        defaultRuntime.log("");
    }
    ...
    if (switchToPackage) {
        ...
        const updateStep = await runUpdateStep({
            name: "global update",
            argv: globalInstallArgs(manager, `moltx@${tag}`),
            timeoutMs: timeoutMs ?? 20 * 60_000,
            progress,
        });
    ...
    if (result.status === "skipped") {
        ...
        if (result.reason === "not-git-install") {
            defaultRuntime.log(theme.warn(`Skipped: this MoltX install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("moltx doctor")}\` and \`${formatCliCommand("moltx gateway restart")}\`.`));
            defaultRuntime.log(theme.muted("Examples: `npm i -g moltx@latest` or `pnpm add -g moltx@latest`"));
        }
        defaultRuntime.exit(0);
        return;
    }
    ...
    if (shouldRestart) {
        ...
        try {
            ...
            if (!opts.json && restarted) {
                ...
                defaultRuntime.log("");
                process.env.MOLTX_UPDATE_IN_PROGRESS = "1";
                try {
                    ...
                    delete process.env.MOLTX_UPDATE_IN_PROGRESS;
                }
            }
        }
        catch (err) {
            if (!opts.json) {
                ...
                defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${formatCliCommand("moltx gateway restart")}`));
            }
        }
    }
    else if (!opts.json) {
        ...
        if (result.mode === "npm" || result.mode === "pnpm") {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("moltx doctor")}\`, then \`${formatCliCommand("moltx gateway restart")}\` to apply updates to a running gateway.`));
        }
        else {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("moltx gateway restart")}\` to apply updates to a running gateway.`));
        }
    }
    if (!opts.json) {
...
export async function updateWizardCommand(opts = {}) {
    if (!process.stdin.isTTY) {
        defaultRuntime.error("Update wizard requires a TTY. Use `moltx update --channel <stable|beta|dev>` instead.");
        defaultRuntime.exit(1);
        return;
    }
    const timeoutMs = opts.timeout ? Number.parseInt(opts.timeout, 10) * 1000 : undefined;
    ...
    if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
        ...
        if (!hasGit) {
            ...
            if (dirExists) {
                ...
                if (!empty) {
                    defaultRuntime.error(`MOLTX_GIT_DIR points at a non-git directory: ${gitDir}. Set MOLTX_GIT_DIR to an empty folder or a moltx checkout.`);
                    defaultRuntime.exit(1);
                    return;
                }
            }
            const ok = await confirm({
                message: stylePromptMessage(`Create a git checkout at ${gitDir}? (override via MOLTX_GIT_DIR)`),
                initialValue: true,
            });
            if (isCancel(ok) || ok === false) {
                defaultRuntime.log(theme.muted("Update cancelled."));
...
export function registerUpdateCli(program) {
    const update = program
        .command("update")
        .description("Update MoltX to the latest version")
        ...
        .addHelpText("after", () => {
        const examples = [
            ["moltx update", "Update a source checkout (git)"],
            ["moltx update --channel beta", "Switch to beta channel (git + npm)"],
            ["moltx update --channel dev", "Switch to dev channel (git + npm)"],
            ["moltx update --tag beta", "One-off update to a dist-tag or version"],
            ["moltx update --no-restart", "Update without restarting the service"],
            ["moltx update --json", "Output result as JSON"],
            ["moltx update --yes", "Non-interactive (accept downgrade prompts)"],
            ["moltx update wizard", "Interactive update wizard"],
            ["moltx --update", "Shorthand for moltx update"],
        ];
...
${theme.heading("Switch channels:")}
  ...
  - Run moltx update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.clawd.bot/cli/update")}`;
    ...
    update
        ...
        .addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
        ["moltx update status", "Show channel + version status."],
        ["moltx update status --json", "JSON output."],
        ["moltx update status --timeout 10", "Custom timeout."],
