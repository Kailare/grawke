...
export function registerDnsCli(program) {
    ...
    dns
        .command("setup")
        .description("Set up CoreDNS to serve moltx.internal for unicast DNS-SD (Wide-Area Bonjour)")
        ...
        defaultRuntime.log("");
        defaultRuntime.log(theme.heading("Recommended ~/.moltx/moltx.json:"));
        ...
        defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): moltx.internal"));
        if (!opts.apply) {
            defaultRuntime.log("");
        ...
        const serverPath = path.join(confDir, "moltx.internal.server");
        ...
        if (zoneFileNeedsBootstrap(zonePath)) {
            ...
            const zoneLines = [
                `; created by moltx dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
        ...
        if (cfg.discovery?.wideArea?.enabled !== true) {
            defaultRuntime.log("");
            defaultRuntime.log(theme.muted("Note: enable discovery.wideArea.enabled in ~/.moltx/moltx.json on the gateway and restart the gateway so it writes the DNS-SD zone."));
        }
    });
}
