...
export function registerDnsCli(program) {
    ...
    dns
        .command("setup")
        .description("Set up CoreDNS to serve grawke.internal for unicast DNS-SD (Wide-Area Bonjour)")
        ...
        defaultRuntime.log("");
        defaultRuntime.log(theme.heading("Recommended ~/.grawke/grawke.json:"));
        ...
        defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): grawke.internal"));
        if (!opts.apply) {
            defaultRuntime.log("");
        ...
        const serverPath = path.join(confDir, "grawke.internal.server");
        ...
        if (zoneFileNeedsBootstrap(zonePath)) {
            ...
            const zoneLines = [
                `; created by grawke dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
        ...
        if (cfg.discovery?.wideArea?.enabled !== true) {
            defaultRuntime.log("");
            defaultRuntime.log(theme.muted("Note: enable discovery.wideArea.enabled in ~/.grawke/grawke.json on the gateway and restart the gateway so it writes the DNS-SD zone."));
        }
    });
}
