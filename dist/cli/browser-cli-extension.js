...
import { STATE_DIR_MOLTX } from "../config/paths.js";
...
function installedExtensionRootDir() {
    return path.join(STATE_DIR_MOLTX, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts) {
    ...
    if (!hasManifest(src)) {
        throw new Error("Bundled Chrome extension is missing. Reinstall MoltX and try again.");
    }
    const stateDir = opts?.stateDir ?? STATE_DIR_MOLTX;
...
export function registerBrowserExtensionCommands(browser, parentOpts) {
    ...
    ext
        ...
        defaultRuntime.error(info([
            ...
            `- Pin “MoltX Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    ext
        ...
        if (!hasManifest(dir)) {
            defaultRuntime.error(danger([
                `Chrome extension is not installed. Run: "${formatCliCommand("moltx browser extension install")}"`,
