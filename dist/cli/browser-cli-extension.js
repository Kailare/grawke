...
import { STATE_DIR_GRAWKE } from "../config/paths.js";
...
function installedExtensionRootDir() {
    return path.join(STATE_DIR_GRAWKE, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts) {
    ...
    if (!hasManifest(src)) {
        throw new Error("Bundled Chrome extension is missing. Reinstall Grawke and try again.");
    }
    const stateDir = opts?.stateDir ?? STATE_DIR_GRAWKE;
...
export function registerBrowserExtensionCommands(browser, parentOpts) {
    ...
    ext
        ...
        defaultRuntime.error(info([
            ...
            `- Pin “Grawke Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    ext
        ...
        if (!hasManifest(dir)) {
            defaultRuntime.error(danger([
                `Chrome extension is not installed. Run: "${formatCliCommand("grawke browser extension install")}"`,
