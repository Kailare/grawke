...
async function setupGatewayTestHome() {
    ...
    previousStateDir = process.env.GRAWKE_STATE_DIR;
    previousConfigPath = process.env.GRAWKE_CONFIG_PATH;
    previousSkipBrowserControl = process.env.GRAWKE_SKIP_BROWSER_CONTROL_SERVER;
    previousSkipGmailWatcher = process.env.GRAWKE_SKIP_GMAIL_WATCHER;
    previousSkipCanvasHost = process.env.GRAWKE_SKIP_CANVAS_HOST;
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "grawke-gateway-home-"));
    process.env.HOME = tempHome;
    process.env.USERPROFILE = tempHome;
    process.env.GRAWKE_STATE_DIR = path.join(tempHome, ".grawke");
    delete process.env.GRAWKE_CONFIG_PATH;
}
function applyGatewaySkipEnv() {
    process.env.GRAWKE_SKIP_BROWSER_CONTROL_SERVER = "1";
    process.env.GRAWKE_SKIP_GMAIL_WATCHER = "1";
    process.env.GRAWKE_SKIP_CANVAS_HOST = "1";
}
async function resetGatewayTestState(options) {
    ...
    tempConfigRoot = options.uniqueConfigRoot
        ? await fs.mkdtemp(path.join(tempHome, "grawke-test-"))
        : path.join(tempHome, ".grawke-test");
...
async function cleanupGatewayTestHome(options) {
    ...
    if (options.restoreEnv) {
        ...
        if (previousStateDir === undefined)
            delete process.env.GRAWKE_STATE_DIR;
        else
            process.env.GRAWKE_STATE_DIR = previousStateDir;
        if (previousConfigPath === undefined)
            delete process.env.GRAWKE_CONFIG_PATH;
        else
            process.env.GRAWKE_CONFIG_PATH = previousConfigPath;
        if (previousSkipBrowserControl === undefined)
            delete process.env.GRAWKE_SKIP_BROWSER_CONTROL_SERVER;
        else
            process.env.GRAWKE_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
        if (previousSkipGmailWatcher === undefined)
            delete process.env.GRAWKE_SKIP_GMAIL_WATCHER;
        else
            process.env.GRAWKE_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
        if (previousSkipCanvasHost === undefined)
            delete process.env.GRAWKE_SKIP_CANVAS_HOST;
        else
            process.env.GRAWKE_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(token, opts) {
    ...
    const prev = process.env.GRAWKE_GATEWAY_TOKEN;
    if (token === undefined) {
        delete process.env.GRAWKE_GATEWAY_TOKEN;
    }
    else {
