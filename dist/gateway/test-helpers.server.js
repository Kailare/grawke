...
async function setupGatewayTestHome() {
    ...
    previousStateDir = process.env.MOLTX_STATE_DIR;
    previousConfigPath = process.env.MOLTX_CONFIG_PATH;
    previousSkipBrowserControl = process.env.MOLTX_SKIP_BROWSER_CONTROL_SERVER;
    previousSkipGmailWatcher = process.env.MOLTX_SKIP_GMAIL_WATCHER;
    previousSkipCanvasHost = process.env.MOLTX_SKIP_CANVAS_HOST;
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "moltx-gateway-home-"));
    process.env.HOME = tempHome;
    process.env.USERPROFILE = tempHome;
    process.env.MOLTX_STATE_DIR = path.join(tempHome, ".moltx");
    delete process.env.MOLTX_CONFIG_PATH;
}
function applyGatewaySkipEnv() {
    process.env.MOLTX_SKIP_BROWSER_CONTROL_SERVER = "1";
    process.env.MOLTX_SKIP_GMAIL_WATCHER = "1";
    process.env.MOLTX_SKIP_CANVAS_HOST = "1";
}
async function resetGatewayTestState(options) {
    ...
    tempConfigRoot = options.uniqueConfigRoot
        ? await fs.mkdtemp(path.join(tempHome, "moltx-test-"))
        : path.join(tempHome, ".moltx-test");
...
async function cleanupGatewayTestHome(options) {
    ...
    if (options.restoreEnv) {
        ...
        if (previousStateDir === undefined)
            delete process.env.MOLTX_STATE_DIR;
        else
            process.env.MOLTX_STATE_DIR = previousStateDir;
        if (previousConfigPath === undefined)
            delete process.env.MOLTX_CONFIG_PATH;
        else
            process.env.MOLTX_CONFIG_PATH = previousConfigPath;
        if (previousSkipBrowserControl === undefined)
            delete process.env.MOLTX_SKIP_BROWSER_CONTROL_SERVER;
        else
            process.env.MOLTX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
        if (previousSkipGmailWatcher === undefined)
            delete process.env.MOLTX_SKIP_GMAIL_WATCHER;
        else
            process.env.MOLTX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
        if (previousSkipCanvasHost === undefined)
            delete process.env.MOLTX_SKIP_CANVAS_HOST;
        else
            process.env.MOLTX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(token, opts) {
    ...
    const prev = process.env.MOLTX_GATEWAY_TOKEN;
    if (token === undefined) {
        delete process.env.MOLTX_GATEWAY_TOKEN;
    }
    else {
