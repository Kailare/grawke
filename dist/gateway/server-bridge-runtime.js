...
export async function startGatewayBridgeRuntime(params) {
    ...
    let bridgeEnabled = (() => {
        ...
        return process.env.GRAWKE_BRIDGE_ENABLED !== "0";
    })();
    const bridgePort = (() => {
        if (typeof params.cfg.bridge?.port === "number" && params.cfg.bridge.port > 0) {
            return params.cfg.bridge.port;
        }
        if (process.env.GRAWKE_BRIDGE_PORT !== undefined) {
            const parsed = Number.parseInt(process.env.GRAWKE_BRIDGE_PORT, 10);
    ...
    const bridgeHost = (() => {
        ...
        if (params.cfg.bridge?.bind === undefined) {
            const env = process.env.GRAWKE_BRIDGE_HOST?.trim();
            if (env)
                return env;
        }
    ...
    const canvasHostPort = (() => {
        if (process.env.GRAWKE_CANVAS_HOST_PORT !== undefined) {
            const parsed = Number.parseInt(process.env.GRAWKE_CANVAS_HOST_PORT, 10);
