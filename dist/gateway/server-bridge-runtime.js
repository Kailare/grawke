...
export async function startGatewayBridgeRuntime(params) {
    ...
    let bridgeEnabled = (() => {
        ...
        return process.env.MOLTX_BRIDGE_ENABLED !== "0";
    })();
    const bridgePort = (() => {
        if (typeof params.cfg.bridge?.port === "number" && params.cfg.bridge.port > 0) {
            return params.cfg.bridge.port;
        }
        if (process.env.MOLTX_BRIDGE_PORT !== undefined) {
            const parsed = Number.parseInt(process.env.MOLTX_BRIDGE_PORT, 10);
    ...
    const bridgeHost = (() => {
        ...
        if (params.cfg.bridge?.bind === undefined) {
            const env = process.env.MOLTX_BRIDGE_HOST?.trim();
            if (env)
                return env;
        }
    ...
    const canvasHostPort = (() => {
        if (process.env.MOLTX_CANVAS_HOST_PORT !== undefined) {
            const parsed = Number.parseInt(process.env.MOLTX_CANVAS_HOST_PORT, 10);
