...
function injectControlUiConfig(html, opts) {
    ...
    const script = `<script>` +
        `window.__MOLTX_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
        `window.__MOLTX_ASSISTANT_NAME__=${JSON.stringify(assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name)};` +
        `window.__MOLTX_ASSISTANT_AVATAR__=${JSON.stringify(assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar)};` +
        `</script>`;
    // Check if already injected
    if (html.includes("__MOLTX_ASSISTANT_NAME__"))
        return html;
