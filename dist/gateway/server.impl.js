...
import { CONFIG_PATH_GRAWKE, isNixMode, loadConfig, migrateLegacyConfig, readConfigFileSnapshot, writeConfigFile, } from "../config/config.js";
...
import { ensureGrawkeCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(port = 18789, opts = {}) {
    ...
    process.env.GRAWKE_GATEWAY_PORT = String(port);
    logAcceptedEnvOption({
        key: "GRAWKE_RAW_STREAM",
        description: "raw stream logging enabled",
    });
    logAcceptedEnvOption({
        key: "GRAWKE_RAW_STREAM_PATH",
    ...
    if (configSnapshot.legacyIssues.length > 0) {
        ...
        if (!migrated) {
            throw new Error(`Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("grawke doctor")}" to migrate.`);
        }
    ...
    if (configSnapshot.exists && !configSnapshot.valid) {
        ...
        throw new Error(`Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("grawke doctor")}" to repair, then retry.`);
    }
