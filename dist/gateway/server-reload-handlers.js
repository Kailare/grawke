    ...
    const applyHotReload = async (plan, nextConfig) => {
        ...
        if (plan.restartGmailWatcher) {
            ...
            if (!isTruthyEnvValue(process.env.MOLTX_SKIP_GMAIL_WATCHER)) {
                ...
                params.logHooks.info("skipping gmail watcher restart (MOLTX_SKIP_GMAIL_WATCHER=1)");
            }
        }
        if (plan.restartChannels.size > 0) {
            if (isTruthyEnvValue(process.env.MOLTX_SKIP_CHANNELS) ||
                isTruthyEnvValue(process.env.MOLTX_SKIP_PROVIDERS)) {
                params.logChannels.info("skipping channel reload (MOLTX_SKIP_CHANNELS=1 or MOLTX_SKIP_PROVIDERS=1)");
            }
            else {
