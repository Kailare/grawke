    ...
    const applyHotReload = async (plan, nextConfig) => {
        ...
        if (plan.restartGmailWatcher) {
            ...
            if (!isTruthyEnvValue(process.env.GRAWKE_SKIP_GMAIL_WATCHER)) {
                ...
                params.logHooks.info("skipping gmail watcher restart (GRAWKE_SKIP_GMAIL_WATCHER=1)");
            }
        }
        if (plan.restartChannels.size > 0) {
            if (isTruthyEnvValue(process.env.GRAWKE_SKIP_CHANNELS) ||
                isTruthyEnvValue(process.env.GRAWKE_SKIP_PROVIDERS)) {
                params.logChannels.info("skipping channel reload (GRAWKE_SKIP_CHANNELS=1 or GRAWKE_SKIP_PROVIDERS=1)");
            }
            else {
