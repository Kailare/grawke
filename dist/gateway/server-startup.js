...
export async function startGatewaySidecars(params) {
    ...
    if (!isTruthyEnvValue(process.env.MOLTX_SKIP_GMAIL_WATCHER)) {
        try {
    ...
    // Tests can opt out via MOLTX_SKIP_CHANNELS (or legacy MOLTX_SKIP_PROVIDERS).
    const skipChannels = isTruthyEnvValue(process.env.MOLTX_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.MOLTX_SKIP_PROVIDERS);
    if (!skipChannels) {
        try {
        ...
        params.logChannels.info("skipping channel start (MOLTX_SKIP_CHANNELS=1 or MOLTX_SKIP_PROVIDERS=1)");
    }
