...
export function requireActiveWebListener(accountId) {
    ...
    if (!listener) {
        throw new Error(`No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`moltx channels login --channel whatsapp --account ${id}`)}.`);
    }
