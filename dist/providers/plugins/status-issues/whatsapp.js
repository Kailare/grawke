    ...
    for (const entry of accounts) {
        ...
        if (!linked) {
            issues.push({
                ...
                fix: "Run: grawke providers login (scan QR on the gateway host).",
            });
            continue;
        }
        if (running && !connected) {
            issues.push({
                ...
                fix: "Run: grawke doctor (or restart the gateway). If it persists, relink via providers login and check logs.",
            });
        }
    }
    return issues;
