    ...
    return guardCancel(await select({
        ...
        options: [
            ...
            },
            {
                ...
                hint: "Delete channel tokens/settings from moltx.json",
            },
        ],
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
    try {
        ...
        intro(opts.command === "update" ? "MoltX update wizard" : "MoltX configure");
        ...
        if (snapshot.exists) {
            ...
            if (!snapshot.valid) {
                outro(`Config invalid. Run \`${formatCliCommand("moltx doctor")}\` to repair it, then re-run configure.`);
                runtime.exit(1);
                return;
            }
        }
        ...
        let gatewayToken = nextConfig.gateway?.auth?.token ??
            ...
            process.env.MOLTX_GATEWAY_TOKEN;
        ...
        if (opts.sections) {
            ...
            if (selected.includes("health")) {
                ...
                const token = nextConfig.gateway?.auth?.token ?? process.env.MOLTX_GATEWAY_TOKEN;
                const password = nextConfig.gateway?.auth?.password ?? process.env.MOLTX_GATEWAY_PASSWORD;
        ...
        const newPassword = nextConfig.gateway?.auth?.password ?? process.env.MOLTX_GATEWAY_PASSWORD;
        const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.MOLTX_GATEWAY_PASSWORD;
        const token = nextConfig.gateway?.auth?.token ?? process.env.MOLTX_GATEWAY_TOKEN;
