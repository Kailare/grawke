    ...
    return guardCancel(await select({
        ...
        options: [
            ...
            },
            {
                ...
                hint: "Delete channel tokens/settings from grawke.json",
            },
        ],
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
    try {
        ...
        intro(opts.command === "update" ? "Grawke update wizard" : "Grawke configure");
        ...
        if (snapshot.exists) {
            ...
            if (!snapshot.valid) {
                outro(`Config invalid. Run \`${formatCliCommand("grawke doctor")}\` to repair it, then re-run configure.`);
                runtime.exit(1);
                return;
            }
        }
        ...
        let gatewayToken = nextConfig.gateway?.auth?.token ??
            ...
            process.env.GRAWKE_GATEWAY_TOKEN;
        ...
        if (opts.sections) {
            ...
            if (selected.includes("health")) {
                ...
                const token = nextConfig.gateway?.auth?.token ?? process.env.GRAWKE_GATEWAY_TOKEN;
                const password = nextConfig.gateway?.auth?.password ?? process.env.GRAWKE_GATEWAY_PASSWORD;
        ...
        const newPassword = nextConfig.gateway?.auth?.password ?? process.env.GRAWKE_GATEWAY_PASSWORD;
        const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.GRAWKE_GATEWAY_PASSWORD;
        const token = nextConfig.gateway?.auth?.token ?? process.env.GRAWKE_GATEWAY_TOKEN;
