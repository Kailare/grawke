...
import { CONFIG_PATH_MOLTX, loadConfig } from "../../config/config.js";
...
export async function modelsStatusCommand(opts, runtime) {
    ...
    runtime.log(`${label("Config")}${colorize(rich, theme.muted, ":")} ${colorize(rich, theme.info, shortenHomePath(CONFIG_PATH_MOLTX))}`);
    ...
    if (missingProvidersInUse.length > 0) {
        ...
        for (const provider of missingProvidersInUse) {
            const hint = provider === "anthropic"
                ? `Run \`claude setup-token\` or \`${formatCliCommand("moltx configure")}\`.`
                : `Run \`${formatCliCommand("moltx configure")}\` or set an API key env var.`;
