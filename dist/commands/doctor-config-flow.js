import { GrawkeSchema, CONFIG_PATH_GRAWKE, migrateLegacyConfig, readConfigFileSnapshot, } from "../config/config.js";
...
function stripUnknownConfigKeys(config) {
    const parsed = GrawkeSchema.safeParse(config);
    if (parsed.success) {
...
export async function loadAndMaybeMigrateDoctorConfig(params) {
    ...
    if (snapshot.legacyIssues.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push(`Run "${formatCliCommand("grawke doctor --fix")}" to apply legacy migrations.`);
        }
    }
    ...
    if (normalized.changes.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push(`Run "${formatCliCommand("grawke doctor --fix")}" to apply these changes.`);
        }
    }
    ...
    if (unknown.removed.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push('Run "grawke doctor --fix" to remove these keys.');
        }
    }
    ...
    return { cfg, path: snapshot.path ?? CONFIG_PATH_GRAWKE, shouldWriteConfig };
}
