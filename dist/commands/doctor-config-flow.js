import { MoltXSchema, CONFIG_PATH_MOLTX, migrateLegacyConfig, readConfigFileSnapshot, } from "../config/config.js";
...
function stripUnknownConfigKeys(config) {
    const parsed = MoltXSchema.safeParse(config);
    if (parsed.success) {
...
export async function loadAndMaybeMigrateDoctorConfig(params) {
    ...
    if (snapshot.legacyIssues.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push(`Run "${formatCliCommand("moltx doctor --fix")}" to apply legacy migrations.`);
        }
    }
    ...
    if (normalized.changes.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push(`Run "${formatCliCommand("moltx doctor --fix")}" to apply these changes.`);
        }
    }
    ...
    if (unknown.removed.length > 0) {
        ...
        if (shouldRepair) {
            ...
            fixHints.push('Run "moltx doctor --fix" to remove these keys.');
        }
    }
    ...
    return { cfg, path: snapshot.path ?? CONFIG_PATH_MOLTX, shouldWriteConfig };
}
