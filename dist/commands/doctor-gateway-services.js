...
export async function maybeMigrateLegacyGatewayService(cfg, mode, runtime, prompter) {
    ...
    const migrate = await prompter.confirmSkipInNonInteractive({
        message: "Migrate legacy gateway services to MoltX now?",
        initialValue: true,
    });
    if (!migrate)
        return;
    ...
    if (loaded) {
        note(`MoltX ${service.label} already ${service.loadedText}.`, "Gateway");
        return;
    }
    const install = await prompter.confirmSkipInNonInteractive({
        message: "Install MoltX gateway service now?",
        initialValue: true,
    });
    if (!install)
        return;
    ...
    const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
        env: process.env,
        port,
        token: cfg.gateway?.auth?.token ?? process.env.MOLTX_GATEWAY_TOKEN,
