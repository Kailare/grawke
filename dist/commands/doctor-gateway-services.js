...
export async function maybeMigrateLegacyGatewayService(cfg, mode, runtime, prompter) {
    ...
    const migrate = await prompter.confirmSkipInNonInteractive({
        message: "Migrate legacy gateway services to Grawke now?",
        initialValue: true,
    });
    if (!migrate)
        return;
    ...
    if (loaded) {
        note(`Grawke ${service.label} already ${service.loadedText}.`, "Gateway");
        return;
    }
    const install = await prompter.confirmSkipInNonInteractive({
        message: "Install Grawke gateway service now?",
        initialValue: true,
    });
    if (!install)
        return;
    ...
    const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
        env: process.env,
        port,
        token: cfg.gateway?.auth?.token ?? process.env.GRAWKE_GATEWAY_TOKEN,
