...
export async function removeChannelConfigWizard(cfg, runtime) {
    ...
    while (true) {
        ...
        if (configured.length === 0) {
            note([
                "No channel config found in grawke.json.",
                `Tip: \`${formatCliCommand("grawke channels status")}\` shows what is configured and enabled.`,
            ].join("\n"), "Remove channel");
            return next;
        }
        ...
        const confirmed = guardCancel(await confirm({
            message: `Delete ${label} configuration from ${shortenHomePath(CONFIG_PATH_GRAWKE)}?`,
            initialValue: false,
        }), runtime);
        if (!confirmed)
            continue;
