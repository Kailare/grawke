...
import { CONFIG_PATH_MOLTX } from "../config/config.js";
...
export async function removeChannelConfigWizard(cfg, runtime) {
    ...
    while (true) {
        ...
        if (configured.length === 0) {
            note([
                "No channel config found in moltx.json.",
                `Tip: \`${formatCliCommand("moltx channels status")}\` shows what is configured and enabled.`,
            ].join("\n"), "Remove channel");
            return next;
        }
        ...
        const confirmed = guardCancel(await confirm({
            message: `Delete ${label} configuration from ${shortenHomePath(CONFIG_PATH_MOLTX)}?`,
            initialValue: false,
        }), runtime);
        if (!confirmed)
            continue;
