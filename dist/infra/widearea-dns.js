...
export const WIDE_AREA_DISCOVERY_DOMAIN = "moltx.internal.";
export const WIDE_AREA_ZONE_FILENAME = "moltx.internal.db";
...
function extractContentHash(zoneText) {
    const match = zoneText.match(/^\s*;\s*moltx-content-hash:\s*(\S+)\s*$/m);
    return match?.[1] ?? null;
}
...
function renderZone(opts) {
    const hostname = os.hostname().split(".")[0] ?? "moltx";
    const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "moltx");
    const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "moltx-gw");
    const txt = [
    ...
    records.push(`_moltx-gw._tcp IN PTR ${instanceLabel}._moltx-gw._tcp`);
    records.push(`${instanceLabel}._moltx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
    records.push(`${instanceLabel}._moltx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
    ...
    return `; moltx-content-hash: ${contentHash}\n${contentBody}`;
}
