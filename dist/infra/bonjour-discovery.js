...
function parseDnsSdBrowse(stdout) {
    ...
    for (const raw of stdout.split("\n")) {
        const line = raw.trim();
        if (!line || !line.includes("_grawke-gw._tcp"))
            continue;
        if (!line.includes("Add"))
            continue;
        const match = line.match(/_grawke-gw\._tcp\.?\s+(.+)$/);
        if (match?.[1]) {
...
async function discoverViaDnsSd(domain, timeoutMs, run) {
    const browse = await run(["dns-sd", "-B", "_grawke-gw._tcp", domain], {
        timeoutMs,
    });
    const instances = parseDnsSdBrowse(browse.stdout);
    const results = [];
    for (const instance of instances) {
        const resolved = await run(["dns-sd", "-L", instance, "_grawke-gw._tcp", domain], {
            timeoutMs,
        });
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
    ...
    ips = ips.slice(0, 40);
    const probeName = `_grawke-gw._tcp.${domain.replace(/\.$/, "")}`;
    const concurrency = 6;
    let nextIndex = 0;
    let nameserver = null;
    let ptrs = [];
    ...
    for (const ptr of ptrs) {
        ...
        if (!ptrName)
            continue;
        const instanceName = ptrName.replace(/\.?_grawke-gw\._tcp\..*$/, "");
...
function parseAvahiBrowse(stdout) {
    ...
    for (const raw of stdout.split("\n")) {
        ...
        if (!line)
            continue;
        if (line.startsWith("=") && line.includes("_grawke-gw._tcp")) {
            if (current)
                results.push(current);
            const marker = " _grawke-gw._tcp";
...
async function discoverViaAvahi(domain, timeoutMs, run) {
    const args = ["avahi-browse", "-rt", "_grawke-gw._tcp"];
