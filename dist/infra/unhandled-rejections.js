...
export function isUnhandledRejectionHandled(reason) {
    for (const handler of handlers) {
        try {
            ...
            console.error("[moltx] Unhandled rejection handler failed:", err instanceof Error ? (err.stack ?? err.message) : err);
        }
    }
    return false;
}
export function installUnhandledRejectionHandler() {
    process.on("unhandledRejection", (reason, _promise) => {
        if (isUnhandledRejectionHandled(reason))
            return;
        console.error("[moltx] Unhandled promise rejection:", formatUncaughtError(reason));
        process.exit(1);
    });
}
