...
export function loadShellEnvFallback(opts) {
    ...
    try {
        ...
        logger.warn(`[moltx] shell env fallback failed: ${msg}`);
        lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
    return isTruthyEnvValue(env.MOLTX_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
    return isTruthyEnvValue(env.MOLTX_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
    const raw = env.MOLTX_SHELL_ENV_TIMEOUT_MS?.trim();
    if (!raw)
