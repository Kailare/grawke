...
export function loadShellEnvFallback(opts) {
    ...
    try {
        ...
        logger.warn(`[grawke] shell env fallback failed: ${msg}`);
        lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
    return isTruthyEnvValue(env.GRAWKE_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
    return isTruthyEnvValue(env.GRAWKE_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
    const raw = env.GRAWKE_SHELL_ENV_TIMEOUT_MS?.trim();
    if (!raw)
