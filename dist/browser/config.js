 ...
 *
 * Note: this is a Grawke browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(cfg) {
    ...
    const envControlUrl = process.env.GRAWKE_BROWSER_CONTROL_URL?.trim();
    const controlToken = cfg?.controlToken?.trim() || undefined;
    const derivedControlPort = (() => {
        const raw = process.env.GRAWKE_GATEWAY_PORT?.trim();
        if (!raw)
            return null;
