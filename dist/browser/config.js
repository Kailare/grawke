 ...
 *
 * Note: this is a MoltX browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(cfg) {
    ...
    const envControlUrl = process.env.MOLTX_BROWSER_CONTROL_URL?.trim();
    const controlToken = cfg?.controlToken?.trim() || undefined;
    const derivedControlPort = (() => {
        const raw = process.env.MOLTX_GATEWAY_PORT?.trim();
        if (!raw)
            return null;
