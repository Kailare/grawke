...
function getBrowserControlToken() {
    const env = process.env.MOLTX_BROWSER_CONTROL_TOKEN?.trim();
    if (env)
        return env;
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    const hint = `Start (or restart) the MoltX gateway (MoltX.app menubar, or \`${formatCliCommand("moltx gateway")}\`) and try again.`;
