...
function getBrowserControlToken() {
    const env = process.env.GRAWKE_BROWSER_CONTROL_TOKEN?.trim();
    if (env)
        return env;
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    const hint = `Start (or restart) the Grawke gateway (Grawke.app menubar, or \`${formatCliCommand("grawke gateway")}\`) and try again.`;
