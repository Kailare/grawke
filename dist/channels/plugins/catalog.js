...
import { discoverMoltXPlugins } from "../../plugins/discovery.js";
...
const ENV_CATALOG_PATHS = ["MOLTX_PLUGIN_CATALOG_PATHS", "MOLTX_MPM_CATALOG_PATHS"];
...
function buildCatalogEntry(candidate) {
    const manifest = candidate.packageMoltX;
    if (!manifest?.channel)
        return null;
...
export function listChannelPluginCatalogEntries(options = {}) {
    const discovery = discoverMoltXPlugins({ workspaceDir: options.workspaceDir });
