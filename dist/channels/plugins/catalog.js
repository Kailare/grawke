...
import { discoverGrawkePlugins } from "../../plugins/discovery.js";
...
const ENV_CATALOG_PATHS = ["GRAWKE_PLUGIN_CATALOG_PATHS", "GRAWKE_MPM_CATALOG_PATHS"];
...
function buildCatalogEntry(candidate) {
    const manifest = candidate.packageGrawke;
    if (!manifest?.channel)
        return null;
...
export function listChannelPluginCatalogEntries(options = {}) {
    const discovery = discoverGrawkePlugins({ workspaceDir: options.workspaceDir });
