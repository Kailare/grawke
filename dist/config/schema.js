...
import { MoltXSchema } from "./zod-schema.js";
const GROUP_LABELS = {
    wizard: "Wizard",
    update: "Update",
...
const FIELD_HELP = {
    "meta.lastTouchedVersion": "Auto-set when MoltX writes the config.",
    ...
    "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /moltx).",
    ...
    "diagnostics.cacheTrace.filePath": "JSONL output path for cache trace logs (default: $MOLTX_STATE_DIR/logs/cache-trace.jsonl).",
    ...
    "agents.defaults.memorySearch.store.path": "SQLite index path (default: ~/.moltx/memory/{agentId}.sqlite).",
    ...
    "plugins.installs": "CLI-managed install metadata (used by `moltx plugins update` to locate install sources).",
    "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
    "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
    "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
    "plugins.installs.*.installPath": "Resolved install directory (usually ~/.moltx/extensions/<id>).",
...
const FIELD_PLACEHOLDERS = {
    ...
    "gateway.controlUi.basePath": "/moltx",
...
function buildBaseConfigSchema() {
    if (cachedBase)
        return cachedBase;
    const schema = MoltXSchema.toJSONSchema({
        target: "draft-07",
        unrepresentable: "any",
    });
    schema.title = "MoltXConfig";
