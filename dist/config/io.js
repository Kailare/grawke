...
import { compareMoltXVersions } from "./version.js";
...
function warnIfConfigFromFuture(cfg, logger) {
    ...
    if (!touched)
        return;
    const cmp = compareMoltXVersions(VERSION, touched);
    if (cmp === null)
        return;
    if (cmp < 0) {
        logger.warn(`Config was last written by a newer MoltX (${touched}); current version is ${VERSION}.`);
    }
}
...
// module scope. `MOLTX_CONFIG_PATH` (and friends) are expected to work even
// when set after the module has been imported (tests, one-off scripts, etc.).
const DEFAULT_CONFIG_CACHE_MS = 200;
let configCache = null;
function resolveConfigCacheMs(env) {
    const raw = env.MOLTX_CONFIG_CACHE_MS?.trim();
    if (raw === "" || raw === "0")
        return 0;
    if (!raw)
...
function shouldUseConfigCache(env) {
    if (env.MOLTX_DISABLE_CONFIG_CACHE?.trim())
        return false;
