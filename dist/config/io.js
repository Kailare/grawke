...
import { compareGrawkeVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
    ...
    "SLACK_BOT_TOKEN",
    "SLACK_APP_TOKEN",
    "GRAWKE_GATEWAY_TOKEN",
    "GRAWKE_GATEWAY_PASSWORD",
];
...
function warnIfConfigFromFuture(cfg, logger) {
    ...
    if (!touched)
        return;
    const cmp = compareGrawkeVersions(VERSION, touched);
    if (cmp === null)
        return;
    if (cmp < 0) {
        logger.warn(`Config was last written by a newer Grawke (${touched}); current version is ${VERSION}.`);
    }
}
...
// module scope. `GRAWKE_CONFIG_PATH` (and friends) are expected to work even
// when set after the module has been imported (tests, one-off scripts, etc.).
const DEFAULT_CONFIG_CACHE_MS = 200;
let configCache = null;
function resolveConfigCacheMs(env) {
    const raw = env.GRAWKE_CONFIG_CACHE_MS?.trim();
    if (raw === "" || raw === "0")
        return 0;
    if (!raw)
...
function shouldUseConfigCache(env) {
    if (env.GRAWKE_DISABLE_CONFIG_CACHE?.trim())
        return false;
