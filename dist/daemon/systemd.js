...
function resolveSystemdServiceName(env) {
    const override = env.MOLTX_SYSTEMD_UNIT?.trim();
    if (override) {
        return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
    }
    return resolveGatewaySystemdServiceName(env.MOLTX_PROFILE);
}
...
export async function installSystemdService({ env, stdout, programArguments, workingDirectory, environment, description, }) {
    ...
    const serviceName = resolveGatewaySystemdServiceName(env.MOLTX_PROFILE);
