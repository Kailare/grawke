...
import { CONFIG_PATH_MOLTX, loadConfig, readConfigFileSnapshot, resolveGatewayPort, validateConfigObjectWithPlugins, writeConfigFile, } from "../config/config.js";
...
export async function runGmailSetup(opts) {
    ...
    if (!configSnapshot.valid) {
        throw new Error(`Config invalid: ${CONFIG_PATH_MOLTX}`);
    }
    ...
    defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_MOLTX)}`);
    defaultRuntime.log(`Next: ${formatCliCommand("moltx webhooks gmail run")}`);
}
