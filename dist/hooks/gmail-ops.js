...
import { CONFIG_PATH_GRAWKE, loadConfig, readConfigFileSnapshot, resolveGatewayPort, validateConfigObjectWithPlugins, writeConfigFile, } from "../config/config.js";
...
export async function runGmailSetup(opts) {
    ...
    if (!configSnapshot.valid) {
        throw new Error(`Config invalid: ${CONFIG_PATH_GRAWKE}`);
    }
    ...
    defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_GRAWKE)}`);
    defaultRuntime.log(`Next: ${formatCliCommand("grawke webhooks gmail run")}`);
}
