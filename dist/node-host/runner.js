...
const execHostEnforced = process.env.GRAWKE_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.GRAWKE_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv() {
    ensureGrawkeCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function runNodeHost(opts) {
    ...
    const token = process.env.GRAWKE_GATEWAY_TOKEN?.trim() ||
...
async function handleInvoke(frame, client, skillBins) {
    ...
    if (command === "browser.proxy") {
        try {
            ...
            const token = process.env.GRAWKE_BROWSER_CONTROL_TOKEN?.trim() || resolved.controlToken?.trim();
            if (token) {
