...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.grawkeCanvasA2UIAction);
  const hasAndroid = () => !!(window.grawkeCanvasA2UIAction && typeof window.grawkeCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.grawkeSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("grawke:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Grawke node canvas.");
      return;
    }
    const ok = window.grawkeSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
    if (isTruthyEnvValue(process.env.GRAWKE_SKIP_CANVAS_HOST))
        return true;
        ...
        try {
            ...
            if (!filePath) {
                if (urlPath === "/" || urlPath.endsWith("/")) {
                    ...
                    res.end(`<!doctype html><meta charset="utf-8" /><title>Grawke Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`);
                    return true;
                }
                res.statusCode = 404;
                res.setHeader("Content-Type", "text/plain; charset=utf-8");
