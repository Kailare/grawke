...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.moltxCanvasA2UIAction);
  const hasAndroid = () => !!(window.moltxCanvasA2UIAction && typeof window.moltxCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.moltxSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("moltx:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android MoltX node canvas.");
      return;
    }
    const ok = window.moltxSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
    if (isTruthyEnvValue(process.env.MOLTX_SKIP_CANVAS_HOST))
        return true;
        ...
        try {
            ...
            if (!filePath) {
                if (urlPath === "/" || urlPath.endsWith("/")) {
                    ...
                    res.end(`<!doctype html><meta charset="utf-8" /><title>MoltX Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`);
                    return true;
                }
                res.statusCode = 404;
                res.setHeader("Content-Type", "text/plain; charset=utf-8");
